...
param = dict(filename_Rho0 = "Inputs/CasTest01/ECMWF_20080901_060000_Latmin-36_Lonmin30_Latmax-16_Lonmax50.txt",
			 ...
			 lambda0 = None,			# Balanced transport if None
			 lambda1 = None,
...
if(((param['lambda0'] is not None) & (param['lambda1'] is None)) |\
	((param['lambda0'] is None) & (param['lambda1'] is not None))):
	print("**** Parameter error : For unbalanced transport, both lambda0 and lambda1 must be set")
	sys.exit(1)
			 
param['name'] = param['name']+'_eps='+str(param['epsilon'])
if(param['lambda0']) is not None:
	param['name'] += "_l0="+str(param['lambda0'])
	if(param['lambda0'] != param['lambda1']):
		param['name'] += "_l1="+str(param['lambda1'])
